const service = ({logger, makeService}) => {
  ...
  svc.on('session:new', (session) => {
    ...
    setTimeout(noMedia.bind(this, session), 5000);

    // after 10 restore full media
    setTimeout(fullMedia.bind(this, session), 10000);

    // after 15 seconds go to partial media
    setTimeout(partialMedia.bind(this, session), 15000);
...
const noMedia = (session) => {
  ...
  session
    .injectCommand('media:path', 'no-media')
    .send();
};
...
const partialMedia = (session) => {
  ...
  session
    .injectCommand('media:path', 'partial-media')
    .send();
};
...
const fullMedia = (session) => {
  ...
  session
    .injectCommand('media:path', 'full-media')
    .send();
};
